...
export RUN_NAME="${MODEL_NAME}_2:4_L200_S50_seqlen_${SEQ_LEN}_bs_${BATCH_SIZE}_grad_accum_${GRAD_ACCUM}_lr_${LR}_min_lr_ratio_${MIN_LR_RATIO}_warmup_ratio_${WARMUP_RATIO}"
if [ ! -z "${TAG:-}" ]; then
...
python -u multihost_runner_orig.py \
    ...
    python3.10 -u -m MaxText.train MaxText/configs/base.yml \
        run_name=${RUN_NAME} \
        load_parameters_path=gs://${BUCKET_NAME}/model_ckpts/maxtext/llama3.1_8b_L200_2:4_0.5/checkpoints/0/items \
