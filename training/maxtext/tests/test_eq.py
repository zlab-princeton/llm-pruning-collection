...
def main(config, test_args):  # pylint: disable=W0621
  ...
  if not test_args.run_hf_model:
    ...
    # HF
    hf_model = AutoModelForCausalLM.from_pretrained(test_args.hf_model_path)
    # ORBAX
    ...
    orbax_model = models.Transformer(config, mesh=mesh, quant=quant)
    state, _ = maxtext_utils.setup_decode_state(orbax_model, config, rng1, mesh, None)
    
    # patch_orbax_weights(hf_model, state, config)
    ...
    for golden_data_index in range(len(golden_data)):
      ...
      # HF
      with torch.no_grad():
        hf_train_logits = hf_model(torch.tensor(ids.tolist())).logits.cpu().numpy().astype("float32")
      hf_train_logits = jax.experimental.multihost_utils.process_allgather(hf_train_logits)
      hf_train_logits = hf_train_logits[:, :seq_len, :]
      # ORBAX
      orbax_train_logits = orbax_model.apply(
        state.params,
        ids,
        decoder_positions,
        decoder_segment_ids,
      ...
      orbax_train_logits = jax.experimental.multihost_utils.process_allgather(orbax_train_logits)
      orbax_train_logits = orbax_train_logits[:, :seq_len, :]

      
      # Slice to original sequence length
      max_logging.log(f"{hf_train_logits[0, 2, :]=}")
      max_logging.log(f"{orbax_train_logits[0, 2, :]=}")
      ...
      max_diff = np.max(np.subtract(hf_train_logits[0, :token_size, :], orbax_train_logits[0, :token_size, :]))
      ...
      hf_probabilities = jax.nn.softmax(hf_train_logits[..., 0, :token_size, :], axis=-1)
      orbax_probabilities = jax.nn.softmax(orbax_train_logits[..., 0, :token_size, :], axis=-1)

      max_logging.log(f"{hf_probabilities[1]=}")
      max_logging.log(f"{orbax_probabilities[1]=}")

      kl_div = jax.numpy.sum(jax.scipy.special.kl_div(hf_probabilities, orbax_probabilities), axis=-1)
      ...
      if test_args.max_kl_div is not None:
        ...
        max_logging.log("Checking Numerical Differences between hf logits and orbax logits")  # pylint: disable=C0301
        assert jax.numpy.allclose(
            hf_train_logits[0, :token_size, :],
            orbax_train_logits[:token_size, :],
    ...
    for input_text in prompts:
      ...
      print("mt_ids.shape", mt_ids.shape)
      print("mt_decoder_positions.shape", mt_decoder_positions.shape)
      print("mt_decoder_segment_ids.shape", mt_decoder_segment_ids.shape)
